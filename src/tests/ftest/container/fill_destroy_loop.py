...
class BoundaryPoolContainerSpace(TestWithServers):
    ...
    def tearDown(self):
        ...
        # DAOS-14141 Check if GC engine silent errors are not occurring during the test.
        err_regex = r"'^.+ tree ERR .+ btr_rec_free.+ Failed to free rec:.+$'"
        cmd = r"find /var/tmp/daos_testing/ -type f -regextype egrep "
        cmd += r"-regex '.*/daos_server\.log\.[[:digit:]]+' "
        cmd += r"-exec grep -E -H -l -e " + err_regex + r" {} +"
    ...
    def write_pool_until_nospace(self, test_loop):
        ...
        if abs(free_space_init - free_space_after_destroy) > self.delta_bytes:
            self.log.warning(
                "Deleting container did not yet restore all free pool space: "
                "loop=%i, init=%s (%i bytes), end=%s (%i bytes)",
                test_loop, bytes_to_human(free_space_init), free_space_init,
                bytes_to_human(free_space_after_destroy), free_space_after_destroy)
            # NOTE Gives more time for space storage info to be updated
            self.log.info("Checking again free pool space in 3 seconds")
            time.sleep(3)
