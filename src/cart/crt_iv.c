...
/* Clone DAOS_BULK_LIMIT for the moment XXX */
#define CRT_IV_BULK_LIMIT (20480 - 1024)
 ...
 */
static int
crt_ivf_finalize(struct iv_fetch_cb_info *iv_info,
		 crt_iv_key_t *iv_key, int output_rc)
{
	crt_rpc_t		*rpc;
	int			 rc = 0;
		  ...
		  struct iv_fetch_cb_info *cb_info)
{
	...
	if (d_sgl_buf_size(iv_value) > CRT_IV_BULK_LIMIT) {
		     ...
		     void *user_priv, int update_rc)
{
	...
	if (iv_value != NULL && d_sgl_buf_size(iv_value) > CRT_IV_BULK_LIMIT) {
...
handle_ivupdate_response(const struct crt_cb_info *cb_info)
{
	...
	if (iv_info->uci_sync_type.ivs_flags & CRT_IV_SYNC_BIDIRECTIONAL) {
		...
		if (input->ivu_iv_value_bulk == CRT_BULK_NULL &&
			...
			child_output->ivo_iv_sgl = output->ivo_iv_sgl;
		}
...
static void
crt_iv_update_inline_hdlr(void *arg)
{
	crt_rpc_t			*rpc = arg;
	struct crt_ivns_internal	*ivns_internal;
	struct crt_iv_ops		*iv_ops;
...
crt_hdlr_iv_update(crt_rpc_t *rpc_req)
{
	...
	if (input->ivu_iv_value_bulk == CRT_BULK_NULL) {
		crt_iv_update_inline_hdlr(rpc_req);
		D_GOTO(exit, rc);
	}
